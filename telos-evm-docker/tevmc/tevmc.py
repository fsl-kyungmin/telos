...
class TEVMController:
        ...
        with self.must_keep_running('kibana'):
            ...
            self.containers['kibana'] = self.exit_stack.enter_context(
                self.open_container(
                    ...
                    f'{config["tag"]}-{self.chain_name}',
                    environment={
                        'ELASTICSEARCH_HOSTS': f'http://elasticsearch:9200',
    ...
    def start_nodeos(
        ...
        with self.must_keep_running('nodeos'):
            ...
            cleos_url = f'http://192.168.50.10:9888'
            ...
            self.containers['telosevm-translator'] = self.exit_stack.enter_context(
                self.open_container(
                    ...
                    environment={
                        # 'CHAIN_NAME': self.chain_name,
                        'CHAIN_ID': 41,
                        'ELASTIC_USERNAME': 'hyper',
                        'ELASTIC_PASSWORD': 'password',
                        'ELASTIC_NODE': f'http://elasticsearch:9200',
                        'TELOS_ENDPOINT': f'http://192.168.50.10:9888',
                        'TELOS_REMOTE_ENDPOINT': f'http://192.168.50.10:9888',
                        'TELOS_WS_ENDPOINT': f'ws://192.168.50.10:29999',
